...
export default class FractalField extends Component {
  ...
  constructor (...args) {
    ...
    this._broadcastChange = debounce(this._broadcastChange.bind(this), 150); // TODO: find correct debounce time
  ...
  _initParentFormListeners () {
    this._parent$.onReceiveMessage(EVENT.SET_CHILD_FIELD_VALUE, (value) => {
      this._setValueEmittedFromParent(value);
    });

    this._parent$.onReceiveMessage(EVENT.FORM_WAS_SUBMITTED, () => {
  ...
  _triggerChange (value) {
    ...
    try {
      ...
      logError(err);
      this._triggerForeignValidationError(this.props.exception, true);
      return;
    }
  ...
  _isValid () {
    ...
    return !state.localError && !state.foreignError && state.validChildren;
  }
  ...
  render () {
    ...
    //console.log(this._fieldName, state.version, state.value);
