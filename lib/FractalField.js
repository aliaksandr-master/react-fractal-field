...
export default class FractalField extends Component {
  ...
  constructor (...args) {
    ...
    this._broadcastChange = debounce(this._broadcastChange.bind(this), 30); // hack for looping exclusion
  }
  ...
  _initChildFieldListeners () {
    this._selfChild$.onReceiveMessage(EVENT.MOUNT_CHILD_FIELD, (fieldID, name, child$) => {
      ...
      child$.onReceiveMessage(EVENT.CHANGE_CHILD_FIELD_VALUE, (normalizedValue, valid) => {
        ...
        this._children.getChildrenByName(name).forEach(({ ee, fieldID: id }) => {
          if (id !== fieldID) {
  ...
  _broadcastChange () { // debounced. see constructor
    const state = this.getState();
    let value = state.normalizedValue;

    const valid = !state.localError && !state.foreignError && this.getState().validChildren;

    if (!valid && !state.foreignError) {
      value = undefined;
    }

    if (isEqual(this._prevVal, value) && this._prevValidity === valid) {
      return;
    }

    this._prevVal = value;
    this._prevValidity = valid;

    this._parent$.sendMessage(EVENT.CHANGE_CHILD_FIELD_VALUE, value, valid);
  }
  ...
  _triggerChange (value) {
    ...
    this.inlineValidate(normalizedValue);
  }

  inlineValidate (normalizedValue) {
    let error;
    ...
    try {
      ...
      logError(err);
      this._triggerForeignValidationError(this.props.exceptionMessage);
      this._broadcastChange();
      return;
    }
  ...
  componentDidUpdate (prevProps) {
    ...
    this.inlineValidate(this.getState().normalizedValue);
  }
