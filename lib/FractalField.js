...
export default class FractalField extends Component {
  ...
  constructor (...args) {
    ...
    this._broadcastChange = debounce(this._broadcastChange.bind(this), 30);
  }
  ...
  _broadcastChange (value, valid) { // made debounce for this function. see constructor
    if (!valid) {
      value = undefined;
    }

    if (isEqual(this._prevVal, value) && this._prevValid === valid) {
      return;
    }
  ...
  onChange (value) {
    ...
    try {
      ...
      this._broadcastChange(undefined, false);
      return;
    }
  ...
  inlineValidate (normalizedValue) {
    ...
    this._broadcastChange(normalizedValue, !error);
  }
