...
export default class FractalField extends Component {
  ...
  dispatch (action) {
    ...
    //console.log('|', this._fieldName, ` ACTION ${action.type}`, action, `, VERSION: ${this._state.version}`, ', VALUE:', this._state.value);
  }
  ...
  _initChildFieldListeners () {
    this._selfChild$.onReceiveMessage(EVENT_MOUNT_CHILD_FIELD, (fieldID, name, child$) => {
      ...
      if (this.getState().submitted) {
        child$.sendMessage(EVENT_FORM_WAS_SUBMITTED);
      }
    ...
    this._selfChild$.onReceiveMessage(EVENT_UNMOUNT_CHILD_FIELD, (fieldID, autoCleanValue) => {
  ...
  _broadcastUnmount () {
    if (this._form$) {
      this._form$.sendMessage(EVENT_UNMOUNT_CHILD_FIELD, this._fieldID, this.props.autoclean);
    }
  }
        ...
        if (isFunction(value)) {
          result[key] = (...args) => {
            if (_children.hasChildren) {
              throw new ReferenceError(`${that._fieldName}: invalid usage of control. ${key}. this element is container and shouldn't have the control.${key}`);
            }
          ...
          Object.defineProperty(result, key, {
            ...
            get () { // eslint-disable-line fp/no-get-set
              if (_children.hasChildren) {
                throw new ReferenceError(`${that._fieldName}: invalid usage of control. ${key}. this element ${that.props.name} is container and shouldn't have the control.${key}`);
              }
