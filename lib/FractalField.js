...
import EE from './EE';
import { change$, initialize$, reset$, submit$ } from './global-streams';
...
export default class FractalField extends ConnectedComponent {
  ...
  _broadcastMount () {
    ...
    if (this._form$) {
      this._form$.postMessage(EVENT_MOUNT_CHILD_FIELD, this._fieldID, this.props.name, this._parent$);
    }
  }

  _broadcastUnmount () {
    if (this._form$) {
      this._form$.postMessage(EVENT_UNMOUNT_CHILD_FIELD, this._fieldID, this.props.autoclean);
    }
  }
  ...
  componentWillReceiveProps (nextProps) {
    ...
    if (this.props.name !== name) {
      ...
      this._parent$.postMessage(EVENT_CHANGE_NAME_OF_CHILD_FIELD, this._fieldID, name);
    }
