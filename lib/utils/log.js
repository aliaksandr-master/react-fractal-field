import { memoize, isError } from '../utils/lodash';
...
export const logDispatch = (window.console && console.log && console.time && console.group && console.timeEnd && console.groupEnd && process.env.NODE_ENV !== 'production') ? (enabled, store, { type, fieldID, payload }) => {
  ...
  const label = `[ReactFractalField] DISPATCH "${type}" (${fieldID})${payload === undefined ? '' : ': '}`;
  ...
  return () => {
    ...
    const payloadArgs = payload === undefined ? [] : [ isError(payload) ? `Error: ${payload.message}` : payload ];

    if (oldVersion !== newVersion) {
      console.group(label, ...payloadArgs);
    } else {
      console.groupCollapsed(label, ...payloadArgs);
    }
