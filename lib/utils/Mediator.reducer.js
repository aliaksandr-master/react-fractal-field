...
const checkSimplicity = (state, fieldID) => {
  const field = state.fields[fieldID];

  if (!field || !field.simple) {
    return;
  }

  const hasNamedChild = state.tree[fieldID].some((childID) => state.fields[childID] && state.fields[childID].name !== null);

  if (hasNamedChild) {
    throw new ReferenceError(`field with ID "${field.id}" marked as simple and must not have named children`);
  }
};
...
const mountReducer = () => (state, { fieldID, payload: { parentID, name = null, ...params } }) => {
  ...
  checkSimplicity(state, fieldID);
  checkSimplicity(state, parentID);
