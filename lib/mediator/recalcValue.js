...
export default recursiveRecalcInSurface((state, field, level, cleanupName = null) => {
  ...
  let val = field.type === TYPE.ARRAY ? [ ...field.value ].filter((item) => !isUndefined(item)) : { ...field.value };

  if (cleanupName !== null && level === 0) {
    val = field.type === TYPE.ARRAY ? [ ...field.value.filter((_1, key) => String(key) !== String(cleanupName)) ] : { ...omit(field.value, String(cleanupName)) };
  }
