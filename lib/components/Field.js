...
import { once, isEqual } from '../utils/lodash';
...
const Field = class Field extends FieldComponent {
  static propTypes = {
    ...
    value: PropTypes.oneOfType([ PropTypes.array, PropTypes.number, PropTypes.string, PropTypes.bool, PropTypes.object ]),
    initialValue: PropTypes.oneOfType([ PropTypes.array, PropTypes.number, PropTypes.string, PropTypes.bool, PropTypes.object ])
  };
  ...
  triggerChange (value, params) {
    value = this.normalize(value);

    super.triggerChange(value, params);
  }
  ...
  onUpdate (prevProps, prevState) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      if (this.mediator.hasNamedChildren()) {
        throw new TypeError(`[FractalField] when type is ${this.valueType}, field "${this.componentID}" must not be complex`);
      }
    }
  }
