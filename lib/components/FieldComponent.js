...
export default class FieldComponent extends Component {
  ...
  constructor (...args) {
    ...
    if ((this.isForm && this.name) || ((this.share === SHARE.META || this.share === SHARE.NONE) && this.name)) {
      throw new Error(`name must not be specified when sharing is "${this.share}"`);
    }
    ...
    this._prevNotifyValueChangeValue = undefined;
  ...
  triggerChange (normalizedValue, { touch = true, notify = true, force = false } = {}) {
    ...
    if (!this.hasExternalValueStorage || force) {
  ...
  _calcParams () {
    if (this._prevCalcParamsMediatorVersion === this.mediator.version() && (this._postponedValueMediatorValueVersion === null || (this._prevPostponedValue === this._postponedValue))) {
  ...
  _refreshIfChanged () {
    ...
    if (this.share !== SHARE.NONE || this._prevRefreshIfChangedMediatorVersion === currentVersion) {
      return;
    }
  ...
  componentDidUpdate (prevProps, prevState) {
    ...
    if (this.hasExternalValueStorage && !isEqual(this.props.value, prevProps.value)) {
      ...
      this.triggerChange(this.props.value, { touch: false, notify: false, force: true });
    }
  ...
  onUpdate (prevProps) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      if (!isNil(val)) {
        assertType(`value of field "${this.componentID}" or null/undefined`, this.getValueType(), val);
      }
    }
