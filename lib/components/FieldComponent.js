...
import { isNumber, isFunction, isEqual, isNil, isUndefined, isError } from '../utils/lodash';
...
const mkID = (parentID, name) =>
  `${salt}${++counter}@${parentID ? `${parentID.replace(/^[^@]+@/, '')}__` : ''}${name == null ? '' : (isNumber(name) ? `[${name}]` : name)}`;


export default class FieldComponent extends Component {
  static propTypes = {
    id: PropTypes.oneOfType([ PropTypes.string, PropTypes.number ]),
    softTypes: PropTypes.bool,
  ...
  _cachedCall (key, val, func) {
    if (this.cached[key] !== val) {
      this.cached[key] = val;
  ...
  getInitialParams () {
    return {
      complexValueType: this.getValueType(),
  ...
  constructor (...args) {
    ...
    this.componentID = this.props.id || mkID(this.hasParentForm ? this.context._fractalFieldMediator.id : '', this.props.name);
  ...
  triggerSubmit (ev) {
    ...
    return Promise.resolve(this.props.onSubmit(value))
      .then(
        (result) => {
          if (isError(result)) {
            this.mediator.submitFailed(result);
  ...
  subscribeGlobalStreams () {
    ...
    this.onDestroy(reset.subscribe(this.componentID, (componentID, value) => this.triggerReset(value)));
  ...
  _refreshIfChanged () {
    if (!this.props.fresh && (this.shareMeta || this.shareValue)) {
      return;
    }

    this._cachedCall('_refreshIfChanged::version', this.mediator.version(), () => {
      if (this.debug) {
